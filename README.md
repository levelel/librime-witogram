...
基于 RIME 引擎的高效 N-gram 语法模型插件（原 `librime-octagram` 架构的深度重构）。

本项目将底层的字典树结构由 `Darts::DoubleArray` 彻底升级为业界标准的 `KenLM`，通过 `-q 8 -b 8` 双重 8-bit 量化，在**完整保留模型 Backoff 回退权重**的同时，全面转向了更科学的**对数线性模型 (Log-Linear Model)** 架构。
...
- **标准 UTF-8 与完整概率分布**：彻底修复了原版模型由于自定义变长编码导致的 `<unk>` 词汇丢失问题。现在的底层是纯正的 UTF-8 字符串，并补全了原版 `.gram` 为节省空间而粗暴抛弃的符合标准概率分布的 Backoff（回退）权重，打分更加科学。
- **动态分词降级 (Word/Char Fallback)**：查询时支持动态回退机制。优先尝试词级别 (Word-level) 匹配；若模型中不存在该词组，则自动降级拆分为单字 (Character-level) 逐字计分。完美向下兼容万象等历史单字模型，且面向未来的词组模型做好了准备。
- **内存映射加载 (mmap)**：在运行时通过 `mmap` 零延迟加载 `.klm` 模型。无论模型物理体积多大（即便因补全概率和转回 UTF-8 后体积有所增加），系统都会按需从磁盘页读取，**带来了极低的驻留内存 (RSS) 占用。不再受模型文件大小的限制，物理内存占用仅取决于实际打字时的活跃词频率。推荐搭配高速 NVMe SSD 使用，以获得最佳的缺页中断响应体验。**
- **纯粹的科学打分**：废弃了原有的各种基于经验的生硬惩罚（如 `collocation_penalty`），直接采用 `KenLM` 返回的平滑对数概率 $\log_{10}(P)$。
- **线程安全与极简架构**：在模型加载器中引入了标准的 `std::mutex` 保证多并发下的线程安全；彻底移除了冗余的 `gram_db` 和 Darts 双数组依赖，插件二进制体积大幅缩减。
...
1. **完整回退算法，更高准确率**：原本的 `.gram` 文件为了节省空间，粗暴丢弃了所有的 Backoff（回退）权重，使用固定惩罚分代替。而 `.klm` 完整保留了符合标准概率分布的 Backoff（回退）权重，打分更科学。
2. **零延迟加载与极低内存占用**：通过内存映射 (`mmap`) 机制，启动输入法时无需等待。操作系统直接按需从磁盘读取模型，带来了极低的驻留内存 (RSS) 占用，物理内存消耗不再受模型文件大小的限制，仅取决于实际打字时的活跃词频率。不再像原来那样将几百兆模型全量塞入内存。推荐搭配高速 NVMe SSD 使用，以获得最佳的缺页中断响应体验。

### 转换步骤

> **💡**：我们已经在 [Releases 页面](../../releases) 提供了预先转换好的万象中文 N-gram 模型。普通用户无需自行编译和执行下方转换步骤，直接下载预制模型文件放入用户目录下 `grammar` 文件夹即可。
